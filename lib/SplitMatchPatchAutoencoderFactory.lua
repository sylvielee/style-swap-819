require 'nn'
local SplitMatchPatchAutoencoderFactory = torch.class('SplitMatchPatchAutoencoderFactory')

-- min patch size is the smallest patch we will split to
-- overhang is how much overlap is added to the edges of each patch
function SplitMatchPatchAutoencoderFactory.buildAutoencoder(target_img, overhang, stride, shuffle, normalize, interpolate)
    local nDim = 3
...
function SplitMatchPatchAutoencoderFactory._build(stride, C, target_patches, npatches, normalize, interpolate)
    ...
    patch_size = 3 -- oh boyYYY ok so this needs to be variable some how 
...
function SplitMatchPatchAutoencoderFactory._extract_patches(img, texture, overhang, stride, shuffle)
    local nDim = 3
    assert(img:nDimension() == nDim, 'image must be of dimension 3.')

    local max_patch_width = min(img:size(2)-(2*overhang), img:size(3)-(2*overhang))
    local min_patch_width = 3 --TODO maybe set to smth else idk 
    local omega = 15 -- this is what the paper set it to

    local regions = torch.tensor([img]) -- first region is simply entire image
    local index = 1
    while index <= regions:size(1) do
        local curr_region = regions[index]
        local std = torch.std(curr_region)

        -- find the texture patch that best matches curr_region

        -- if it's not good enough
        -- split into four
        -- add those four to regions
        -- use the code below to make regions = the four new regions + whatever was in regions - curr_region
        -- do this with gather()
        -- the indices we want to keep
        local i = index
        x = torch.Tensor(regions:size(1)):apply(function(x)
            i = i + 1
            return i
        end)

        -- ELSE
        -- increment index so that this region is no longer split

        -- to do the "pyramid" don't remove the old big ones even if we split them
    end

    -- save the partitions in a file with torch.save and then visualize them with some python script
...
function _find_best_patch(region, texture, patch_width)
